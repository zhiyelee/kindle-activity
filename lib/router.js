...
// API
var searchRouter = kr.get('/search', function *() {
  // get the list match the key words
  var key = this.request.query.key;

  if (!key) {
    this.body = 'No keywords are specified.';
    return;
  }
  this.body = yield util.searchItems(key);
});
router.add(searchRouter);
